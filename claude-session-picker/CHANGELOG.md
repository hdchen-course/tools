...
- **A stale picker-owned tmux server is recovered, not stuck.** If an *older*
  ...
  socket path. The reader is BOUNDED by a size check (`stat` the file, refuse
  anything over 96 bytes before reading a single byte) plus a capped read, so a
  corrupt/huge owner file — even a NUL-heavy one, which Bash 4.3+ would otherwise
  scan to EOF because it skips NULs past `read -n` — can never stall startup on
  any supported bash (3.2 through 5.x, macOS and Linux).
